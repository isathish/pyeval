...
def corpus_wer(
    ...
    corpus_wer_value = (
        (total_subs + total_dels + total_ins) / total_ref_len
        if total_ref_len > 0
        else 0.0
    )
...
def character_error_rate(
    reference: str, hypothesis: str, include_spaces: bool = False
) -> Dict[str, Any]:
    """
...
def corpus_cer(
    references: List[str], hypotheses: List[str], include_spaces: bool = False
) -> Dict[str, Any]:
    """
...
def match_error_rate(reference: str, hypothesis: str) -> Dict[str, float]:
    ...
    return {"mer": mer, "wer": result["wer"], "total_alignments": total_alignments}
...
def align_texts(reference: str, hypothesis: str) -> List[Dict[str, Any]]:
        ...
        for j in range(1, h_len + 1):
            if ref_words[i - 1] == hyp_words[j - 1]:
                ...
                costs = [
                    (
                        d[i - 1][j - 1] + 1,
                        ("substitute", ref_words[i - 1], hyp_words[j - 1]),
                    ),
    ...
    while i > 0 or j > 0:
        ...
        alignment.append({"operation": op[0], "reference": op[1], "hypothesis": op[2]})
...
def slot_error_rate(
    reference_slots: Dict[str, str], hypothesis_slots: Dict[str, str]
) -> Dict[str, Any]:
    """
...
def intent_accuracy(
    reference_intents: List[str], hypothesis_intents: List[str]
) -> Dict[str, float]:
    """
...
def phoneme_error_rate(
    reference_phonemes: List[str], hypothesis_phonemes: List[str]
) -> Dict[str, Any]:
    """
...
def diarization_error_rate(
    ...
    if total_ref_time <= 0:
        return {"der": 0.0, "false_alarm": 0.0, "miss": 0.0, "speaker_error": 0.0}
...
def mean_opinion_score(
    scores: List[float], confidence_interval: float = 0.95
) -> Dict[str, float]:
    """
    ...
    """
    if not scores:
        return {"mos": 0.0, "std": 0.0, "ci_lower": 0.0, "ci_upper": 0.0, "count": 0}
...
def fluency_score(
    reference: str, hypothesis: str, expected_rate: float = 150.0
) -> Dict[str, float]:
    """
...
class SpeechMetrics:
    ...
    @classmethod
    def compute_corpus(
        cls, references: List[str], hypotheses: List[str]
    ) -> "SpeechMetrics":
        """
        ...
        mer_values = [
            match_error_rate(r, h)["mer"] for r, h in zip(references, hypotheses)
        ]
        wil_values = [
            word_information_lost(r, h)["wil"] for r, h in zip(references, hypotheses)
        ]
        accuracy_values = [
            recognition_accuracy(r, h) for r, h in zip(references, hypotheses)
        ]
