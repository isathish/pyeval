...
def demographic_parity(
    y_pred: List[int], sensitive_features: List[Any], favorable_outcome: int = 1
) -> Dict[str, Any]:
    """
...
def equalized_odds(
    ...
    for group in groups:
        ...
        tp = sum(
            1
            for t, p in zip(group_true, group_pred)
...
def disparate_impact(
    ...
    if not group_rates:
        return {"di_ratio": 1.0, "has_disparate_impact": False, "group_ratios": {}}
...
def calibration_by_group(
    ...
    for group in groups:
        ...
        group_calibration[group] = {"ece": ece, "mce": mce}
...
def individual_fairness(
    X: List[List[float]], y_pred: List[int], distance_threshold: float = 0.1
) -> Dict[str, float]:
    """
...
def counterfactual_fairness_check(
    predictions_original: List[int], predictions_counterfactual: List[int]
) -> Dict[str, Any]:
    """
    ...
    flips = sum(
        1 for o, c in zip(predictions_original, predictions_counterfactual) if o != c
    )
...
def comprehensive_fairness_analysis(
    ...
    results = {
        "demographic_parity": demographic_parity(
            y_pred, sensitive_features, favorable_outcome
        ),
        "equalized_odds": equalized_odds(
            y_true, y_pred, sensitive_features, favorable_outcome
        ),
    ...
    if y_scores:
        results["calibration"] = calibration_by_group(
            y_true, y_scores, sensitive_features
        )
