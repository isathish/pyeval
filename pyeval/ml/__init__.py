...
def accuracy_score(
    y_true: List[Any], y_pred: List[Any], normalize: bool = True
) -> float:
    """
...
def precision_score(
    ...
    if average is None:
        ...
        fp_sum = sum(
            sum(matrix[j][i] for j in range(len(labels))) - matrix[i][i]
...
def true_positive_rate(
    y_true: List[int], y_pred: List[int], pos_label: int = 1
) -> float:
    """
...
def balanced_accuracy_score(
    y_true: List[int], y_pred: List[int], adjusted: bool = False
) -> float:
    """
...
def log_loss(
    y_true: List[int], y_prob: List[Union[float, List[float]]], eps: float = 1e-15
) -> float:
    """
...
def jaccard_score(
    ...
    if average == "binary":
        intersection = sum(
            1 for t, p in zip(y_true, y_pred) if t == pos_label and p == pos_label
        )
        union = sum(
            1 for t, p in zip(y_true, y_pred) if t == pos_label or p == pos_label
        )
    ...
    for label in labels:
        ...
        union = sum(1 for t, p in zip(y_true, y_pred) if t == label or p == label)
...
def classification_report(
    ...
    weighted_prec = (
        sum(
            report_dict[str(label)]["precision"] * report_dict[str(label)]["support"]
            for label in labels
        )
        / total_support
        if total_support > 0
    ...
    weighted_rec = (
        sum(
            report_dict[str(label)]["recall"] * report_dict[str(label)]["support"]
            for label in labels
        )
        / total_support
        if total_support > 0
    ...
    weighted_f1 = (
        sum(
            report_dict[str(label)]["f1-score"] * report_dict[str(label)]["support"]
            for label in labels
        )
        / total_support
        if total_support > 0
    ...
    rows.append(
        ["accuracy", "", "", f"{report_dict['accuracy']:.2f}", str(len(y_true))]
    )
    ...
    col_widths = [
        max(len(str(row[i])) for row in [headers] + rows) + 2 for i in range(5)
    ]
...
def mean_squared_error(
    y_true: List[float], y_pred: List[float], squared: bool = True
) -> float:
    """
...
def mean_absolute_percentage_error(
    y_true: List[float], y_pred: List[float], epsilon: float = 1e-10
) -> float:
    """
...
def adjusted_r2_score(
    y_true: List[float], y_pred: List[float], n_features: int
) -> float:
    """
...
def quantile_loss(
    y_true: List[float], y_pred: List[float], quantile: float = 0.5
) -> float:
    """
...
def normalized_rmse(
    y_true: List[float], y_pred: List[float], method: str = "std"
) -> float:
    """
...
def silhouette_score(X: List[List[float]], labels: List[int]) -> float:
    ...
    for i in range(n_samples):
        ...
        same_cluster = [
            j for j in range(n_samples) if labels[j] == cluster_i and j != i
        ]
        if not same_cluster:
            a_i = 0.0
        else:
...
def davies_bouldin_score(X: List[List[float]], labels: List[int]) -> float:
    ...
    for i, cluster_i in enumerate(unique_labels):
        ...
        for j, cluster_j in enumerate(unique_labels):
            ...
            # R_ij = (S_i + S_j) / M_ij
            centroid_dist = euclidean_distance(
                centroids[cluster_i], centroids[cluster_j]
            )
...
def calinski_harabasz_score(X: List[List[float]], labels: List[int]) -> float:
    ...
    # Overall centroid
    overall_centroid = [
        mean([X[i][f] for i in range(n_samples)]) for f in range(n_features)
    ]
    ...
    for cluster in unique_labels:
        ...
        # Cluster centroid
        cluster_centroid = [
            mean([X[i][f] for i in cluster_indices]) for f in range(n_features)
        ]
...
def adjusted_rand_score(labels_true: List[int], labels_pred: List[int]) -> float:
    ...
    sum_b = sum(
        ...
        * (sum(contingency[i][j] for i in range(len(classes))) - 1)
        // 2
...
def normalized_mutual_info_score(
    labels_true: List[int], labels_pred: List[int], average_method: str = "arithmetic"
) -> float:
    """
    ...
    for (t, p), count in contingency.items():
        if count > 0:
            mi += (count / n) * math.log(
                (count * n) / (true_counts[t] * pred_counts[p])
            )
...
def homogeneity_score(labels_true: List[int], labels_pred: List[int]) -> float:
    ...
    for cluster in clusters:
        ...
        cluster_entropy = -sum(
            (count / cluster_size) * math.log(count / cluster_size)
...
def v_measure_score(
    labels_true: List[int], labels_pred: List[int], beta: float = 1.0
) -> float:
    """
