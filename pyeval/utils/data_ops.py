...
def stratified_split(
    data: List[Any], labels: List[Any], test_size: float = 0.2, random_seed: int = 42
...
def k_fold_indices(
    n_samples: int, k: int = 5, shuffle: bool = True, random_seed: int = 42
...
def one_hot_encode(
    labels: List[int], n_classes: Optional[int] = None
) -> List[List[int]]:
    """
...
def sample_weights_from_class_weights(
    labels: List[Any], class_weights: Dict[Any, float]
) -> List[float]:
    """
...
def bootstrap_sample(
    data: List[Any], n_samples: Optional[int] = None, random_seed: int = 42
