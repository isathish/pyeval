...
def bootstrap_confidence_interval(
    ...
    stat_funcs = {"mean": mean, "median": median, "std": lambda x: std(x, ddof=1)}
...
def independent_t_test(
    sample1: List[Number], sample2: List[Number], equal_var: bool = True
) -> Dict[str, float]:
    """
...
def wilcoxon_signed_rank_test(
    sample1: List[Number], sample2: List[Number]
) -> Dict[str, float]:
    """
...
def mcnemar_test(table: List[List[int]]) -> Dict[str, float]:
    ...
    return {"chi_square": chi_sq, "p_value": p_value, "b": b, "c": c}
