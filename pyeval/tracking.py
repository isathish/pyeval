...
class ExperimentTracker:
    ...
    def create_run(
        self, experiment_name: str, tags: Optional[List[str]] = None
    ) -> ExperimentRun:
        """
    ...
    def compare_runs(
        self, run_ids: Optional[List[str]] = None, experiment_name: Optional[str] = None
    ) -> str:
        """
        ...
        for run in runs:
            lines.append(
                f"{run.run_id:<25} | {run.experiment_name:<20} | {run.status:<10}"
            )
        ...
        if all_params:
            ...
            lines.append(
                f"{'Parameter':<25} | "
                + " | ".join(f"{r.run_id[:15]:<15}" for r in runs)
            )
            lines.append("-" * 80)
        ...
        if all_metrics:
            lines.append("\n--- Metrics ---")
            lines.append(
                f"{'Metric':<25} | " + " | ".join(f"{r.run_id[:15]:<15}" for r in runs)
            )
            lines.append("-" * 80)
        ...
        for exp_name in self.list_experiments():
            ...
            if exp_runs:
                ...
                for metric in sorted(metrics):
                    values = [
                        r.metrics.get(metric) for r in exp_runs if metric in r.metrics
                    ]
                    if values:
