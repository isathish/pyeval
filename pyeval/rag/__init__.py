...
def context_relevance(
    question: str, contexts: List[str], method: str = "keyword"
) -> Dict[str, Any]:
    """
    ...
    """
    if not contexts:
        return {"overall_relevance": 0.0, "context_scores": [], "relevant_contexts": 0}
...
def context_precision(
    question: str, contexts: List[str], relevant_threshold: float = 0.3
) -> float:
    """
...
def context_recall(
    question: str, contexts: List[str], ground_truth_contexts: List[str]
) -> float:
    """
...
def answer_correctness(
    answer: str, ground_truth: str, question: Optional[str] = None
) -> Dict[str, float]:
    """
    ...
    if not answer_tokens or not truth_tokens:
        return {"correctness": 0.0, "precision": 0.0, "recall": 0.0, "f1": 0.0}
...
def retrieval_precision(
    retrieved: List[str], relevant: List[str], k: Optional[int] = None
) -> float:
    """
...
def groundedness_score(answer: str, contexts: List[str]) -> Dict[str, Any]:
    ...
    for sent in sentences:
        ...
        if sent_entities:
            entity_grounding = len(sent_entities & context_entities) / len(
                sent_entities
            )
        else:
        ...
        if grounding >= 0.3:
            ...
            ungrounded.append({"sentence": sent, "grounding_score": grounding})
...
def rag_faithfulness(answer: str, contexts: List[str]) -> Dict[str, Any]:
    ...
    for sent in sentences:
        ...
        details.append(
            {"sentence": sent, "support": max_support, "faithful": is_faithful}
        )
...
def noise_robustness(
    question: str, answer: str, contexts: List[str], noise_contexts: List[str]
) -> Dict[str, float]:
    """
...
def context_entity_recall(contexts: List[str], ground_truth: str) -> Dict[str, float]:
    ...
    def extract_entities(text: str) -> Set[str]:
        ...
        entities.update(
            re.findall(
                r"\b\d+(?:\.\d+)?(?:\s*(?:km|m|kg|lb|%|year|day|hour)s?)?\b", text
            )
        )
        # Quoted strings
...
def answer_attribution(answer: str, contexts: List[str]) -> Dict[str, Any]:
    ...
    used_contexts = set(
        a["context_index"] for a in attributions if a["context_index"] is not None
    )
...
def context_utilization(answer: str, contexts: List[str]) -> Dict[str, float]:
    ...
    for i, ctx in enumerate(contexts):
        ...
        utilizations.append({"context_index": i, "utilization": utilization})

    # Overall metrics
    avg_utilization = (
        mean([u["utilization"] for u in utilizations]) if utilizations else 0.0
    )
    max_utilization = (
        max([u["utilization"] for u in utilizations]) if utilizations else 0.0
    )
...
def question_answer_relevance(question: str, answer: str) -> Dict[str, float]:
    ...
    if question_type == "yes_no":
        if any(
            w in answer_lower
            for w in ["yes", "no", "correct", "incorrect", "true", "false"]
        ):
            type_match_score = 1.0
        else:
...
def rag_pipeline_score(
    question: str, contexts: List[str], answer: str, ground_truth: Optional[str] = None
) -> Dict[str, float]:
    """
...
def context_coverage(
    question: str, contexts: List[str], required_aspects: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
    ...
    ctx_entities = set(
        re.findall(r"\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b", combined_context)
    )
    ...
    # Overall coverage
    coverage = (
        question_term_coverage * 0.4 + aspect_coverage * 0.3 + entity_coverage * 0.3
    )
...
def faithful_answer_rate(
    answers: List[str], contexts_list: List[List[str]], threshold: float = 0.5
) -> Dict[str, Any]:
    """
    ...
    for i, (answer, contexts) in enumerate(zip(answers, contexts_list)):
        ...
        per_answer.append(
            {"index": i, "faithfulness": faithfulness, "is_faithful": is_faithful}
        )
