...
from pyeval.utils.math_ops import jaccard_similarity, mean
...
def hallucination_score(
    ...
    for claim in claims:
        ...
        if support < threshold:
            hallucinated.append({"claim": claim, "support_score": support})
        else:
            supported.append({"claim": claim, "support_score": support})
...
def answer_relevancy(
    ...
    relevancy = (
        keyword_coverage * 0.4 + semantic_overlap * 0.4 + type_bonus + 0.1
    )  # Base score
...
def faithfulness_score(
    response: str, source: str, granularity: str = "sentence"
) -> Dict[str, Any]:
    """
    ...
    for unit in units:
        ...
        if unit_faithfulness >= 0.4:
            ...
            unfaithful.append({"text": unit, "faithfulness": unit_faithfulness})
...
def coherence_score(text: str) -> Dict[str, float]:
    ...
    if keyword_freq:
        ...
        recurring_ratio = sum(1 for f in keyword_freq.values() if f > 1) / len(
            keyword_freq
        )
        global_coherence = min(
            1.0, (max_freq / len(sentences)) * 0.5 + recurring_ratio * 0.5
        )
    else:
    ...
    # Final coherence
    coherence = min(
        1.0, (local_coherence * 0.4 + global_coherence * 0.4 + transition_bonus + 0.2)
    )  # Base score
...
def consistency_score(
    responses: List[str], question: Optional[str] = None
) -> Dict[str, float]:
    """
    ...
    for i in range(len(responses)):
        for j in range(i + 1, len(responses)):
            if response_tokens[i] and response_tokens[j]:
                jaccard = len(response_tokens[i] & response_tokens[j]) / len(
                    response_tokens[i] | response_tokens[j]
                )
        ...
        if all_facts:
            ...
            for i in range(len(responses)):
                for j in range(i + 1, len(responses)):
                    ...
                    if all_pair_facts:
                        fact_agreement_scores.append(
                            len(common_facts) / len(all_pair_facts)
                        )
            factual_consistency = (
                mean(fact_agreement_scores) if fact_agreement_scores else 0.5
            )
        else:
            factual_consistency = 1.0
    else:
    ...
    # Overall consistency
    consistency = (
        factual_consistency * 0.4 + semantic_similarity * 0.35 + lexical_overlap * 0.25
    )
...
def completeness_score(
    question: str, answer: str, expected_aspects: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
    ...
    # Final completeness
    completeness = term_coverage * 0.3 + aspect_coverage * 0.4 + length_adequacy * 0.3
...
def fluency_score(text: str) -> Dict[str, float]:
    ...
    # Final fluency
    fluency = length_score * 0.3 + vocabulary_richness * 0.3 + grammar_score * 0.4
...
def bias_detection(
    text: str, bias_categories: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
...
def instruction_following_score(instruction: str, response: str) -> Dict[str, float]:
    ...
    if instruction_tokens:
        topic_coverage = len(instruction_tokens & response_tokens) / len(
            instruction_tokens
        )
    else:
        topic_coverage = 0.5
...
def multi_turn_coherence(conversation: List[Dict[str, str]]) -> Dict[str, float]:
    ...
    if len(conversation) < 2:
        return {"coherence": 1.0, "topic_consistency": 1.0, "reference_tracking": 1.0}

    # Extract assistant responses
    responses = [
        turn["content"] for turn in conversation if turn.get("role") == "assistant"
    ]
    ...
    for i in range(1, len(responses)):
        prev_tokens = (
            set(tokenize(responses[i - 1], lowercase=True, remove_punct=True))
            - STOPWORDS
        )
        curr_tokens = (
            set(tokenize(responses[i], lowercase=True, remove_punct=True)) - STOPWORDS
        )
...
def summarization_quality(source: str, summary: str) -> Dict[str, float]:
    ...
    redundancy = (
        sum(1 for c in summary_counts.values() if c > 1) / len(summary_set)
        if summary_set
        else 0
    )
