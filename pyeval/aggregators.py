...
class CrossValidationAggregator:
    ...
    def get_summary(self) -> Dict[str, Dict[str, float]]:
        ...
        for metric_name in metric_names:
            ...
            mean = sum(values) / n
            std = (
                math.sqrt(sum((x - mean) ** 2 for x in values) / (n - 1))
                if n > 1
                else 0
            )
    ...
    def get_best_fold(self, metric_name: str, maximize: bool = True) -> FoldResult:
        ...
        def key_func(fr):
            return fr.metrics.get(
                metric_name, float("-inf") if maximize else float("inf")
            )

        return (
            max(self._fold_results, key=key_func)
            if maximize
            else min(self._fold_results, key=key_func)
        )
...
class EnsembleAggregator:
    ...
    def __init__(
        self, strategy: str = "majority_vote", weights: Dict[str, float] = None
    ):
