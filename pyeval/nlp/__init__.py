...
from pyeval.utils.text_ops import tokenize, ngrams, stem, STOPWORDS
...
def _modified_precision(
    reference_ngrams: List[Counter], candidate_ngrams: Counter
) -> Tuple[int, int]:
    """
...
def _brevity_penalty(reference_lengths: List[int], candidate_length: int) -> float:
    ...
    closest_ref_len = min(
        reference_lengths, key=lambda r: (abs(r - candidate_length), r)
    )
...
def bleu_score(
    ...
    for ref in references:
        if isinstance(ref, str):
            reference_tokens_list.append(
                tokenize(ref, lowercase=True, remove_punct=True)
            )
        elif isinstance(ref, list):
            # Already tokenized
            reference_tokens_list.append(
                [t.lower() if isinstance(t, str) else t for t in ref]
            )
    ...
    for n in range(1, max_n + 1):
        ...
        reference_ngrams = [
            Counter(ngrams(ref_tokens, n)) for ref_tokens in reference_tokens_list
        ]
    ...
    return {
        ...
        "reference_length": min(
            reference_lengths, key=lambda r: abs(r - len(candidate_tokens))
        ),
    }
...
def corpus_bleu(
    references: List[List[str]], candidates: List[str], max_n: int = 4
) -> Dict[str, float]:
    """
    ...
    for refs, cand in zip(references, candidates):
        ...
        ref_tokens_list = [
            tokenize(ref, lowercase=True, remove_punct=True) for ref in refs
        ]

        for n in range(1, max_n + 1):
            cand_ngrams = Counter(ngrams(cand_tokens, n))
            ref_ngrams = [
                Counter(ngrams(ref_tokens, n)) for ref_tokens in ref_tokens_list
            ]
...
def rouge_score(
    reference: str, candidate: str, rouge_types: Optional[List[str]] = None
    ...
    for rouge_type in rouge_types:
        ...
        results[rouge_type] = {"precision": precision, "recall": recall, "f1": f1}
...
def _stem_match(
    ...
    ref_matched: Set[int],
    cand_matched: Set[int],
) -> Tuple[Set[int], Set[int]]:
    """Find stem matches between unmatched tokens."""
    ref_remaining = {
        i: stem(t) for i, t in enumerate(ref_tokens) if i not in ref_matched
    }
...
def _synonym_match(
    ...
    ref_remaining = {
        i: ref_tokens[i] for i in range(len(ref_tokens)) if i not in ref_matched
    }
...
def meteor_score(
    ...
    ref_matched, cand_matched = _stem_match(
        ref_tokens, cand_tokens, ref_matched, cand_matched
    )

    # Stage 3: Synonym matches
    ref_matched, cand_matched = _synonym_match(
        ref_tokens, cand_tokens, ref_matched, cand_matched
    )
...
def ter_score(reference: str, candidate: str) -> Dict[str, float]:
    ...
    return {"ter": ter, "edit_distance": edit_dist, "reference_length": len(ref_tokens)}
...
def chrf_score(
    reference: str, candidate: str, n: int = 6, beta: float = 2.0
) -> Dict[str, float]:
    """
    ...
    return {"chrf": chrf, "precision": avg_precision, "recall": avg_recall}
...
def lexical_diversity(text: str) -> Dict[str, float]:
    ...
    maas = (
        (math.log(n_tokens) - math.log(n_types)) / (math.log(n_tokens) ** 2)
        if n_tokens > 1
        else 0
    )
...
class NLPMetrics:
    ...
    @classmethod
    def compute_batch(
        cls, references: List[str], candidates: List[str]
    ) -> "NLPMetrics":
        """
        ...
        metrics_list = [
            cls.compute(ref, cand) for ref, cand in zip(references, candidates)
        ]
