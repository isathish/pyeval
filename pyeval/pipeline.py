...
class Pipeline:
    ...
    def run(
        self, y_true: List, y_pred: List, return_details: bool = False
    ) -> Union[Dict[str, float], "PipelineResult"]:
        ...
        try:
            ...
            for metric in self._metrics:
                ...
                self._call_hooks(
                    "after_metric", metric.name, metric_results.get(metric._name)
                )
...
def create_classification_pipeline() -> Pipeline:
    ...
    return (
        ...
        .validate(
            lambda x: len(x[0]) == len(x[1]), "y_true and y_pred must have same length"
        )
        .validate(lambda x: len(x[0]) > 0, "Input arrays cannot be empty")
        .add_metric("accuracy", accuracy_score)
