...
class EarlyStoppingCallback(Callback):
    ...
    def __init__(
        self, metric: str, mode: str = "min", patience: int = 5, min_delta: float = 0.0
    ):
        self.metric = metric
        self.mode = mode
    ...
    def on_metric_end(self, context: CallbackContext):
        ...
        if improved:
            ...
            if self.counter >= self.patience:
                ...
                print(
                    f"Early stopping triggered: {self.metric} did not improve for {self.patience} evaluations"
                )
...
class CallbackEvaluator:
    ...
    def evaluate(
        self, y_true: List, y_pred: List, metrics: Dict[str, Callable]
    ) -> Dict[str, float]:
        ...
        try:
            for name, metric_func in metrics.items():
                ...
                self.callback_manager.dispatch(
                    CallbackContext(
                        event=CallbackEvent.ON_METRIC_START, metric_name=name
                    )
                )
                ...
                try:
                    ...
                    self.callback_manager.dispatch(
                        CallbackContext(
                            event=CallbackEvent.ON_ERROR, metric_name=name, error=e
                        )
                    )
                    raise
        finally:
            ...
            self.callback_manager.dispatch(
                CallbackContext(
                    event=CallbackEvent.ON_EVALUATION_END, metadata={"results": results}
                )
            )
