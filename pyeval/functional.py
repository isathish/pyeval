...
def reduce_list(func: Callable[[T, T], T], lst: List[T], 
...
def zip_with(func: Callable[[T, U], Any], 
             lst1: List[T], lst2: List[U]) -> List[Any]:
...
def combine_metrics(*metric_funcs: Callable) -> Callable:
    ...
    def combined_metric(y_true: List, y_pred: List) -> dict:
        return {
            func.__name__: func(y_true, y_pred)
            for func in metric_funcs
        }
    return combined_metric


def threshold_metric(metric_func: Callable, threshold: float, 
                     above: bool = True) -> Callable:
    """
...
def average_metric(metric_func: Callable, 
                   samples: List[Tuple[List, List]]) -> float:
    """
...
def weighted_average_metric(metric_func: Callable,
