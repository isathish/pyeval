...
class Evaluator:
    ...
    def evaluate_regression(
        self, y_true: List[float], y_pred: List[float], name: str = "Regression"
    ) -> EvaluationReport:
        """
    ...
    def evaluate_generation(
        self, references: List[str], hypotheses: List[str], name: str = "Generation"
    ) -> EvaluationReport:
        """
        ...
        """
        from pyeval.nlp import sentence_bleu, rouge_score, meteor_score, distinct_n
    ...
    def evaluate_llm(
        ...
        """
        from pyeval.llm import toxicity_score, coherence_score, fluency_score
        ...
        coherence_scores = [
            coherence_score(resp, query) for query, resp in zip(queries, responses)
        ]
        ...
        report.add_metric(
            "avg_response_length", mean([len(r.split()) for r in responses])
        )
        ...
        if contexts:
            ...
            faith_scores = [
                faithfulness_score(resp, ctx) for resp, ctx in zip(responses, contexts)
            ]
    ...
    def evaluate_rag(
        ...
        """
        from pyeval.rag import context_relevance, groundedness_score, rag_faithfulness
        ...
        # Context relevance
        ctx_rel_scores = [
            context_relevance(query, ctx) for query, ctx in zip(queries, contexts)
        ]
    ...
    def evaluate_fairness(
        ...
        """
        from pyeval.fairness import demographic_parity, equalized_odds, disparate_impact

        report = EvaluationReport(name=name, domain="fairness")

        report.add_metric(
            "demographic_parity_diff", demographic_parity(y_pred, sensitive_features)
        )
        report.add_metric(
            "equalized_odds_diff", equalized_odds(y_true, y_pred, sensitive_features)
        )
        report.add_metric(
            "disparate_impact", disparate_impact(y_pred, sensitive_features)
        )
    ...
    def evaluate_speech(
        self, references: List[str], hypotheses: List[str], name: str = "Speech"
    ) -> EvaluationReport:
        """
        ...
        wer_scores = [
            word_error_rate(ref, hyp) for ref, hyp in zip(references, hypotheses)
        ]
        cer_scores = [
    ...
    def evaluate_recommender(
        ...
        report.add_metric(
            f"precision@{k}", mean_precision_at_k(recommendations, relevants, k)
        )
        report.add_metric(
            f"recall@{k}", mean_recall_at_k(recommendations, relevants, k)
        )
        report.add_metric(f"ndcg@{k}", mean_ndcg_at_k(recommendations, relevants, k))
        report.add_metric("map", mean_average_precision(recommendations, relevants))
        report.add_metric(f"hit_rate@{k}", mean_hit_rate(recommendations, relevants, k))
...
class ExperimentTracker:
    ...
    def start_experiment(self, name: str, description: str = "") -> Experiment:
        """
    ...
    def compare_experiments(self, names: Optional[List[str]] = None) -> str:
        ...
        lines.append("\n--- Metrics ---")
        lines.append(
            f"{'Metric':<25} | " + " | ".join(f"{e.name:<15}" for e in experiments)
        )
        lines.append("-" * 80)
