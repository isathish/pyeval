...
class TestPatterns(unittest.TestCase):
    ...
    def test_strategy_pattern(self):
        ...
        from pyeval.patterns import (
            MetricCalculator, AccuracyStrategy, F1Strategy
        )
    ...
    def test_observer_pattern(self):
        ...
        from pyeval.patterns import (
            MetricSubject, LoggingObserver, Event, EventData
        )
...
class TestValidators(unittest.TestCase):
    ...
    def test_schema_validator(self):
        ...
        from pyeval.validators import (
            SchemaValidator, FieldSchema, ListValidator, NumericValidator
        )
...
class TestCallbacks(unittest.TestCase):
    ...
    def test_callback_manager(self):
        ...
        from pyeval.callbacks import (
            CallbackManager, LoggingCallback, CallbackContext, CallbackEvent
        )
    ...
    def test_threshold_callback(self):
        ...
        from pyeval.callbacks import (
            ThresholdCallback, CallbackContext, CallbackEvent
        )
        ...
        # Above threshold
        callback.on_metric_end(CallbackContext(
            event=CallbackEvent.ON_METRIC_END,
            metric_name='accuracy',
            metric_value=0.9
        ))
    ...
    def test_history_callback(self):
        ...
        from pyeval.callbacks import (
            HistoryCallback, CallbackContext, CallbackEvent
        )
        
        callback = HistoryCallback()
        
        callback.on_evaluation_start(CallbackContext(event=CallbackEvent.ON_EVALUATION_START))
        callback.on_metric_end(CallbackContext(
            event=CallbackEvent.ON_METRIC_END,
            metric_name='accuracy',
            metric_value=0.95
        ))
        callback.on_evaluation_end(CallbackContext(event=CallbackEvent.ON_EVALUATION_END))
    ...
    def test_aggregation_callback(self):
        """Test aggregation."""
        from pyeval.callbacks import (
            AggregationCallback, CallbackContext, CallbackEvent
        )
...
class TestFunctional(unittest.TestCase):
    ...
    def test_higher_order_functions(self):
        ...
        from pyeval.functional import (
            map_list, filter_list, reduce_list, group_by
        )
...
class TestAggregators(unittest.TestCase):
    ...
    def test_basic_aggregators(self):
        ...
        from pyeval.aggregators import (
            MeanAggregator, MedianAggregator, MinAggregator, MaxAggregator
        )
...
class TestIntegration(unittest.TestCase):
    ...
    def test_full_pipeline_with_callbacks(self):
        ...
        pipeline = (
            Pipeline()
            .add_metric('accuracy', mock_accuracy)
            .add_hook('after_metric', lambda name, val: aggregator.add_result(name, val) if val else None)
        )
